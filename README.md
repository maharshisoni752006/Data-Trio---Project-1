...
# Description
This is a simple console-based Snake game implemented in C++. The player controls a snake that moves around the grid, eating food to grow in size while avoiding collisions with the walls and itself. The game continues until the snake collides with a wall or itself, at which point the player can restart or exit. The code is OS-dependant.
...
3. The snake moves in one of four directions (UP, DOWN, LEFT, RIGHT), controlled by the player using the `W`, `A`, `S`, and `D` keys.
...
- C++ Standard Library
  - `iostream`, `vector`, `deque`, `ctime`, etc.
- Console-based graphics
  - ASCII rendering of the grid
- Windows system calls(OS-dependant)
  - `system("cls")` for clearing the screen, `_kbhit()` and `_getch()` for keyboard input,   `Sleep()` for controlling game speed
